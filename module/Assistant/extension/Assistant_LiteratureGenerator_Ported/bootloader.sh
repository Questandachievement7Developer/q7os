...
if [ $Option == "reset" ]
    ...
    rm -rf data/artists/*
    mkdir data/artists
    bash /Assistant/extension/Assistant_LiteratureGenerator_Ported/code/db_reset.sh
fi
if [ $Option == "generate" ]
	then
      ...
      "Slot10" "Make New Literature based on Existing Writer Slot 10" )
   		 SlotSelection=$(dialog --clear \
                ...
                "${OPTIONS[@]}" \
                2>&1 >/dev/tty)
	  	cd /Assistant/extension/Assistant_LiteratureGenerator_Ported/code
	  dialog --title "$A" --msgbox 'Generating...' 5 50
      Length=$(dialog --title "Literature Length Option" --inputbox "How long should it be generate" 8 40 2>&1 >/dev/tty)
      #zenity --info --text='Generating...'
      sleep 4
      echo '10'
      echo '# Outputing Words'
      python3.5 generate.py $SlotSelection $Length
      echo '60' ; sleep 3
      cp -r /Assistant/extension/Assistant_LiteratureGenerator_Ported/final.txt /sdcard
      echo '# Finalizing'
      cd /Assistant/extension/Assistant_LiteratureGenerator_Ported
      echo '100' ; sleep 3
      echo '# done'
...
if [ $Option == "read" ]
		then
			nano /Assistant/extension/Assistant_LiteratureGenerator_Ported/final.txt
fi
