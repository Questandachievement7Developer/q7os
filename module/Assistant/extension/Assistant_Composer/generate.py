...
print('[Orion_torentoMusic]: Please Wait While we initializing the brain part')
import numpy as np
import pandas as pd
#import msgpack
import glob
...
#Import All detected midis
os.system('cp -rv **/*.mid* Music_Midi')
###################################################
# In order for this code to work, you need to place this file in the same
# directory as the midi_manipulation.py file and the Music_Midi directory
...
print(("{} [Orion]songs processed".format(len(songs))))
###################################################
...
num_timesteps  = randint(3, 27)  #15 #This is the number of timesteps that we will create at a time
n_visible      = 2*note_range*num_timesteps #This is the size of the visible layer.
n_hidden       = randint(3, 50)#(100, 10000) #50 #This is the size of the hidden layer # adjusts Speed (Optimization Key)
print(num_timesteps)
print(n_hidden)
...
num_epochs = randint(17000, 75600) #The number of training epochs that we are going to run. For each epoch we go through the entire data set.
batch_size = randint(64, 6000) #The number of training examples that we are going to send through the RBM at a time.# adjusts Memory Usage
lr         = tf.constant(0.0012, tf.float32) #The learning rate of our model

#print(randint(0,100))
#print ('[Orion_torentoMusic]: epoch length decided' )
#print(num_epochs)
#print ('[Orion_torentoMusic]: learning strategies decided' )
#print(batch_size)
### Variables:
# Next, let's look at the variables we're going to use:

x  = tf.placeholder(tf.float32, [None, n_visible], name="x") #The placeholder variable that holds our data
W  = tf.Variable(tf.random_normal([n_visible, n_hidden], 1.213278620111204), name="W") #The weight matrix that stores the edge weights
...
os.system('python2 merge_samples.py')
os.system('rm gen*.mi*')
os.system('rm tag*.*')
