...
for i in $a ; do for proc in $(pgrep $i) ;do chrt -a -p 10 $proc ;renice -n 20 -p $proc ; ionice -c 3 -p $proc ; done ;done
...
for proc in $(pgrep gdm) ;do chrt -a -p 99 $proc ;renice -n -20 -p $proc ;ionice -c 0 -p $proc ;sleep 0.1; done
...
for proc in $(pgrep Xorg) ;do chrt -a -p 99 $proc ; renice -n -20 -p $proc ; ionice -c 0 -p $proc ;sleep 0.1; done
for proc in $(pgrep term) ;do chrt -a -p 99 $proc ;renice -n -20 -p $proc ; ionice -c 0 -p $proc ;sleep 0.3; done
for proc in $(pgrep modem) ;do chrt -a -p 99 $proc ;renice -n -20 -p $proc ; ionice -c 0 -p $proc ;sleep 0.3; done
sleep 60
done
